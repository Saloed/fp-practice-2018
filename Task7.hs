...
Ф = 1.6 * max(insize, outsize)
Ф(0) = 0 
При добавлениии в начало или конец потенциал увеличивается на 1
При удалении из начала или конца потенциал уменьшается на 1
Перекидывание уменьшает потенциал следующим образом:
   Пусть размер наибольшего списка равен maxsize. Тогда при  c = 4

   Тогда при перекидывании maxsize изменится
   как maxsize / 1.6
   Следовательно, потенциал после перекидывания будет равен
   1.6 * maxsize / 1.6 = maxsize
